...
#include "ISXEQMoveUtils.h"

// The mandatory pre-setup function.  Our name is "ISXEQMoveUtils", and the class is ISXEQMoveUtils.
...
// our instanced class.
ISXPreSetup("ISXEQMoveUtils", ISXEQMoveUtils);
...
unsigned int ISXEQMoveUtilsXML = 0;
...
bool ISXEQMoveUtils::Initialize(ISInterface *p_ISInterface)
{
	...
	WriteChatf("ISXEQMoveUtils Loaded");
	return true;
}

// shutdown sequence
void ISXEQMoveUtils::Shutdown()
{
	// save settings, if you changed them and want to save them now.  You should normally save
	// changes immediately.
	//pISInterface->ExportSet(ISXEQMoveUtilsXML,XMLFileName);

	pISInterface->UnloadSet(ISXEQMoveUtilsXML);
...
void ISXEQMoveUtils::ConnectServices()
{
...
void ISXEQMoveUtils::RegisterCommands()
{
	// add any commands
...
void ISXEQMoveUtils::RegisterAliases()
{
	// add any aliases
}

void ISXEQMoveUtils::RegisterDataTypes()
{
	// add any datatypes
	// pMyType = new MyType;
...
void ISXEQMoveUtils::UnRegisterCommands()
{
	// remove commands
	//    pISInterface->RemoveCommand("MyCommand");
}
void ISXEQMoveUtils::UnRegisterAliases()
{
	// remove aliases
}
void ISXEQMoveUtils::UnRegisterDataTypes()
{
	// remove data types
	//if (pMyType)
	//{
...
void ISXEQMoveUtils::UnRegisterTopLevelObjects()
{
	// remove Top-Level Objects
	//    pISInterface->RemoveTopLevelObject("MapSpawn");
}
void ISXEQMoveUtils::UnRegisterServices()
{
	// shutdown our own services
	//    if (hISXEQMoveUtilsService)
	//        pISInterface->ShutdownService(this,hISXEQMoveUtilsService);
}

void ISXEQMoveUtils::LoadSettings()
{
	bool bChanged = false;

	// open the settings file, and store the ID number in ISXEQMoveUtilsXML
	ISXEQMoveUtilsXML = pISInterface->OpenSettings(XMLFileName);
	...
	Any set can be referred to by its ID.  The base set is stored in ISXEQMoveUtilsXML.  Note that the settings
	...
	*/
	...
	if (unsigned int GeneralGUID = pISInterface->FindSet(ISXEQMoveUtilsXML, "General"))
	{
