...
#include "ISXEQRelay.h"

// The mandatory pre-setup function.  Our name is "ISXEQRelay", and the class is ISXEQRelay.
...
// our instanced class.
ISXPreSetup("ISXEQRelay", ISXEQRelay);
...
bool ISXEQRelay::Initialize(ISInterface *p_ISInterface)
{
	...
	WriteChatf("ISXEQRelay Loaded");
	return true;
}

// shutdown sequence
void ISXEQRelay::Shutdown()
{
	// save settings, if you changed them and want to save them now.  You should normally save
	// changes immediately.
	//pISInterface->ExportSet(ISXEQRelayXML,XMLFileName);

	pISInterface->UnloadSet(ISXEQRelayXML);
...
void ISXEQRelay::ConnectServices()
{
...
void ISXEQRelay::RegisterCommands()
{
	// add any commands
...
void ISXEQRelay::RegisterAliases()
{
	// add any aliases
}

void ISXEQRelay::RegisterDataTypes()
{
	// add any datatypes
	// pMyType = new MyType;
...
void ISXEQRelay::UnRegisterCommands()
{
	// remove commands
	//    pISInterface->RemoveCommand("MyCommand");
}
void ISXEQRelay::UnRegisterAliases()
{
	// remove aliases
}
void ISXEQRelay::UnRegisterDataTypes()
{
	// remove data types
	//if (pMyType)
	//{
...
void ISXEQRelay::UnRegisterTopLevelObjects()
{
	// remove Top-Level Objects
	//    pISInterface->RemoveTopLevelObject("MapSpawn");
}
void ISXEQRelay::UnRegisterServices()
{
	// shutdown our own services
	//    if (hISXEQRelayService)
	//        pISInterface->ShutdownService(this,hISXEQRelayService);
}

void ISXEQRelay::LoadSettings()
{
	bool bChanged = false;

	// open the settings file, and store the ID number in ISXEQRelayXML
	ISXEQRelayXML = pISInterface->OpenSettings(XMLFileName);
	...
	Any set can be referred to by its ID.  The base set is stored in ISXEQRelayXML.  Note that the settings
	...
	*/
	...
	if (unsigned int GeneralGUID = pISInterface->FindSet(ISXEQRelayXML, "General"))
	{
