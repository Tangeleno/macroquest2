...
#include "ISXEQAdvPath.h"

// The mandatory pre-setup function.  Our name is "ISXEQAdvPath", and the class is ISXEQAdvPath.
...
// our instanced class.
ISXPreSetup("ISXEQAdvPath", ISXEQAdvPath);
...
unsigned int ISXEQAdvPathXML = 0;
...
bool ISXEQAdvPath::Initialize(ISInterface *p_ISInterface)
{
	...
	WriteChatf("ISXEQAdvPath Loaded");
	return true;
}

// shutdown sequence
void ISXEQAdvPath::Shutdown()
{
	// save settings, if you changed them and want to save them now.  You should normally save
	// changes immediately.
	//pISInterface->ExportSet(ISXEQAdvPathXML,XMLFileName);

	pISInterface->UnloadSet(ISXEQAdvPathXML);
...
void ISXEQAdvPath::ConnectServices()
{
...
void ISXEQAdvPath::RegisterCommands()
{
	// add any commands
...
void ISXEQAdvPath::RegisterAliases()
{
	// add any aliases
}

void ISXEQAdvPath::RegisterDataTypes()
{
	// add any datatypes
	// pMyType = new MyType;
...
void ISXEQAdvPath::UnRegisterCommands()
{
	// remove commands
	//    pISInterface->RemoveCommand("MyCommand");
}
void ISXEQAdvPath::UnRegisterAliases()
{
	// remove aliases
}
void ISXEQAdvPath::UnRegisterDataTypes()
{
	// remove data types
	//if (pMyType)
	//{
...
void ISXEQAdvPath::UnRegisterTopLevelObjects()
{
	// remove Top-Level Objects
	//    pISInterface->RemoveTopLevelObject("MapSpawn");
}
void ISXEQAdvPath::UnRegisterServices()
{
	// shutdown our own services
	//    if (hISXEQAdvPathService)
	//        pISInterface->ShutdownService(this,hISXEQAdvPathService);
}

void ISXEQAdvPath::LoadSettings()
{
	bool bChanged = false;

	// open the settings file, and store the ID number in ISXEQAdvPathXML
	ISXEQAdvPathXML = pISInterface->OpenSettings(XMLFileName);
	...
	Any set can be referred to by its ID.  The base set is stored in ISXEQAdvPathXML.  Note that the settings
	...
	*/
	...
	if (unsigned int GeneralGUID = pISInterface->FindSet(ISXEQAdvPathXML, "General"))
	{
