...
#include "ISXEQAutoforage.h"

// The mandatory pre-setup function.  Our name is "ISXEQAutoforage", and the class is ISXEQAutoforage.
...
// our instanced class.
ISXPreSetup("ISXEQAutoforage", ISXEQAutoforage);
...
unsigned int ISXEQAutoforageXML = 0;
...
bool ISXEQAutoforage::Initialize(ISInterface *p_ISInterface)
{
	...
	WriteChatf("ISXEQAutoforage Loaded");
	return true;
}

// shutdown sequence
void ISXEQAutoforage::Shutdown()
{
	// save settings, if you changed them and want to save them now.  You should normally save
	// changes immediately.
	//pISInterface->ExportSet(ISXEQAutoforageXML,XMLFileName);

	pISInterface->UnloadSet(ISXEQAutoforageXML);
...
void ISXEQAutoforage::ConnectServices()
{
...
void ISXEQAutoforage::RegisterCommands()
{
	// add any commands
...
void ISXEQAutoforage::RegisterAliases()
{
	// add any aliases
}

void ISXEQAutoforage::RegisterDataTypes()
{
	// add any datatypes
	// pMyType = new MyType;
...
void ISXEQAutoforage::UnRegisterCommands()
{
	// remove commands
	//    pISInterface->RemoveCommand("MyCommand");
}
void ISXEQAutoforage::UnRegisterAliases()
{
	// remove aliases
}
void ISXEQAutoforage::UnRegisterDataTypes()
{
	// remove data types
	//if (pMyType)
	//{
...
void ISXEQAutoforage::UnRegisterTopLevelObjects()
{
	// remove Top-Level Objects
	//    pISInterface->RemoveTopLevelObject("MapSpawn");
}
void ISXEQAutoforage::UnRegisterServices()
{
	// shutdown our own services
	//    if (hISXEQAutoforageService)
	//        pISInterface->ShutdownService(this,hISXEQAutoforageService);
}

void ISXEQAutoforage::LoadSettings()
{
	bool bChanged = false;

	// open the settings file, and store the ID number in ISXEQAutoforageXML
	ISXEQAutoforageXML = pISInterface->OpenSettings(XMLFileName);
	...
	Any set can be referred to by its ID.  The base set is stored in ISXEQAutoforageXML.  Note that the settings
	...
	*/
	...
	if (unsigned int GeneralGUID = pISInterface->FindSet(ISXEQAutoforageXML, "General"))
	{
