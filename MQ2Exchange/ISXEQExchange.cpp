...
unsigned int ISXEQExchangeXML = 0;
...
void ISXEQExchange::Shutdown()
{
	...
	// changes immediately.
	//pISInterface->ExportSet(ISXEQExchangeXML,XMLFileName);

	pISInterface->UnloadSet(ISXEQExchangeXML);
...
void ISXEQExchange::LoadSettings()
{
	...
	// open the settings file, and store the ID number in ISXEQExchangeXML
	ISXEQExchangeXML = pISInterface->OpenSettings(XMLFileName);
	...
	Any set can be referred to by its ID.  The base set is stored in ISXEQExchangeXML.  Note that the settings
	...
	*/
	...
	if (unsigned int GeneralGUID = pISInterface->FindSet(ISXEQExchangeXML, "General"))
	{
