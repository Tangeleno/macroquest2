...
#include "ISXEQTwist.h"

// The mandatory pre-setup function.  Our name is "ISXEQTwist", and the class is ISXEQTwist.
...
// our instanced class.
ISXPreSetup("ISXEQTwist", ISXEQTwist);
...
bool ISXEQTwist::Initialize(ISInterface *p_ISInterface)
{
	...
	WriteChatf("ISXEQTwist Loaded");
	return true;
}

// shutdown sequence
void ISXEQTwist::Shutdown()
{
	// save settings, if you changed them and want to save them now.  You should normally save
	// changes immediately.
	//pISInterface->ExportSet(ISXEQTwistXML,XMLFileName);

	pISInterface->UnloadSet(ISXEQTwistXML);
...
void ISXEQTwist::ConnectServices()
{
...
void ISXEQTwist::RegisterCommands()
{
	// add any commands
...
void ISXEQTwist::RegisterAliases()
{
	// add any aliases
}

void ISXEQTwist::RegisterDataTypes()
{
	// add any datatypes
	// pMyType = new MyType;
...
void ISXEQTwist::UnRegisterCommands()
{
	// remove commands
	//    pISInterface->RemoveCommand("MyCommand");
}
void ISXEQTwist::UnRegisterAliases()
{
	// remove aliases
}
void ISXEQTwist::UnRegisterDataTypes()
{
	// remove data types
	//if (pMyType)
	//{
...
void ISXEQTwist::UnRegisterTopLevelObjects()
{
	// remove Top-Level Objects
	//    pISInterface->RemoveTopLevelObject("MapSpawn");
}
void ISXEQTwist::UnRegisterServices()
{
	// shutdown our own services
	//    if (hISXEQTwistService)
	//        pISInterface->ShutdownService(this,hISXEQTwistService);
}

void ISXEQTwist::LoadSettings()
{
	bool bChanged = false;

	// open the settings file, and store the ID number in ISXEQTwistXML
	ISXEQTwistXML = pISInterface->OpenSettings(XMLFileName);
	...
	Any set can be referred to by its ID.  The base set is stored in ISXEQTwistXML.  Note that the settings
	...
	*/
	...
	if (unsigned int GeneralGUID = pISInterface->FindSet(ISXEQTwistXML, "General"))
	{
