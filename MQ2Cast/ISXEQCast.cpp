...
// The mandatory pre-setup function.  Our name is "ISXEQCast", and the class is ISXEQCast.
...
// our instanced class.
ISXPreSetup("ISXEQCast", ISXEQCast);
...
bool ISXEQCast::Initialize(ISInterface *p_ISInterface)
{
	...
	WriteChatf("ISXEQCast Loaded");
	return true;
}

// shutdown sequence
void ISXEQCast::Shutdown()
{
	// save settings, if you changed them and want to save them now.  You should normally save
	// changes immediately.
	//pISInterface->ExportSet(ISXEQCastXML,XMLFileName);

	pISInterface->UnloadSet(ISXEQCastXML);
...
void ISXEQCast::ConnectServices()
{
...
void ISXEQCast::RegisterCommands()
{
	// add any commands
...
void ISXEQCast::RegisterAliases()
{
	// add any aliases
}

void ISXEQCast::RegisterDataTypes()
{
	// add any datatypes
	// pMyType = new MyType;
...
void ISXEQCast::UnRegisterCommands()
{
	// remove commands
	//    pISInterface->RemoveCommand("MyCommand");
}
void ISXEQCast::UnRegisterAliases()
{
	// remove aliases
}
void ISXEQCast::UnRegisterDataTypes()
{
	// remove data types
	//if (pMyType)
	//{
...
void ISXEQCast::UnRegisterTopLevelObjects()
{
	// remove Top-Level Objects
	//    pISInterface->RemoveTopLevelObject("MapSpawn");
}
void ISXEQCast::UnRegisterServices()
{
	// shutdown our own services
	//    if (hISXEQCastService)
	//        pISInterface->ShutdownService(this,hISXEQCastService);
}

void ISXEQCast::LoadSettings()
{
	bool bChanged = false;

	// open the settings file, and store the ID number in ISXEQCastXML
	ISXEQCastXML = pISInterface->OpenSettings(XMLFileName);
	...
	Any set can be referred to by its ID.  The base set is stored in ISXEQCastXML.  Note that the settings
	...
	*/
	...
	if (unsigned int GeneralGUID = pISInterface->FindSet(ISXEQCastXML, "General"))
	{
