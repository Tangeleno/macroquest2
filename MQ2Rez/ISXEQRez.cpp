...
// The mandatory pre-setup function.  Our name is "ISXEQRez", and the class is ISXEQRez.
...
bool ISXEQRez::Initialize(ISInterface *p_ISInterface)
{
	...
	WriteChatf("ISXEQRez Loaded");
	return true;
}

// shutdown sequence
void ISXEQRez::Shutdown()
{
	// save settings, if you changed them and want to save them now.  You should normally save
	// changes immediately.
	//pISInterface->ExportSet(ISXEQRezXML,XMLFileName);

	pISInterface->UnloadSet(ISXEQRezXML);
...
void ISXEQRez::RegisterCommands()
{
	// add any commands
...
void ISXEQRez::RegisterAliases()
{
	// add any aliases
}

void ISXEQRez::RegisterDataTypes()
{
	// add any datatypes
	// pMyType = new MyType;
...
void ISXEQRez::UnRegisterCommands()
{
	// remove commands
	//    pISInterface->RemoveCommand("MyCommand");
}
void ISXEQRez::UnRegisterAliases()
{
	// remove aliases
}
void ISXEQRez::UnRegisterDataTypes()
{
	// remove data types
	//if (pMyType)
	//{
...
void ISXEQRez::UnRegisterTopLevelObjects()
{
	// remove Top-Level Objects
	//    pISInterface->RemoveTopLevelObject("MapSpawn");
}
void ISXEQRez::UnRegisterServices()
{
	// shutdown our own services
	//    if (hISXEQRezService)
	//        pISInterface->ShutdownService(this,hISXEQRezService);
}

void ISXEQRez::LoadSettings()
{
	bool bChanged = false;

	// open the settings file, and store the ID number in ISXEQRezXML
	ISXEQRezXML = pISInterface->OpenSettings(XMLFileName);
	...
	Any set can be referred to by its ID.  The base set is stored in ISXEQRezXML.  Note that the settings
	...
	*/
	...
	if (unsigned int GeneralGUID = pISInterface->FindSet(ISXEQRezXML, "General"))
	{
