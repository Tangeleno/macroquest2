...
#include "ISXEQTangSM.h"

// The mandatory pre-setup function.  Our name is "ISXEQTangSM", and the class is ISXEQTangSM.
...
// our instanced class.
ISXPreSetup("ISXEQTangSM", ISXEQTangSM);
...
bool ISXEQTangSM::Initialize(ISInterface *p_ISInterface)
{
	...
	WriteChatf("ISXEQTangSM Loaded");
	return true;
}

// shutdown sequence
void ISXEQTangSM::Shutdown()
{
	// save settings, if you changed them and want to save them now.  You should normally save
	// changes immediately.
	//pISInterface->ExportSet(ISXEQTangSMXML,XMLFileName);

	pISInterface->UnloadSet(ISXEQTangSMXML);
...
void ISXEQTangSM::ConnectServices()
{
...
void ISXEQTangSM::RegisterCommands()
{
	// add any commands
...
void ISXEQTangSM::RegisterAliases()
{
	// add any aliases
}

void ISXEQTangSM::RegisterDataTypes()
{
	// add any datatypes
	// pMyType = new MyType;
...
void ISXEQTangSM::UnRegisterCommands()
{
	// remove commands
	//    pISInterface->RemoveCommand("MyCommand");
}
void ISXEQTangSM::UnRegisterAliases()
{
	// remove aliases
}
void ISXEQTangSM::UnRegisterDataTypes()
{
	// remove data types
	//if (pMyType)
	//{
...
void ISXEQTangSM::UnRegisterTopLevelObjects()
{
	// remove Top-Level Objects
	//    pISInterface->RemoveTopLevelObject("MapSpawn");
}
void ISXEQTangSM::UnRegisterServices()
{
	// shutdown our own services
	//    if (hISXEQTangSMService)
	//        pISInterface->ShutdownService(this,hISXEQTangSMService);
}

void ISXEQTangSM::LoadSettings()
{
	bool bChanged = false;

	// open the settings file, and store the ID number in ISXEQTangSMXML
	ISXEQTangSMXML = pISInterface->OpenSettings(XMLFileName);
	...
	Any set can be referred to by its ID.  The base set is stored in ISXEQTangSMXML.  Note that the settings
	...
	*/
	...
	if (unsigned int GeneralGUID = pISInterface->FindSet(ISXEQTangSMXML, "General"))
	{
