...
// The mandatory pre-setup function.  Our name is "ISXEQFPS", and the class is ISXEQFPS.
...
bool ISXEQFPS::Initialize(ISInterface *p_ISInterface)
{
	...
	WriteChatf("ISXEQFPS Loaded");
	return true;
}

// shutdown sequence
void ISXEQFPS::Shutdown()
{
	// save settings, if you changed them and want to save them now.  You should normally save
	// changes immediately.
	//pISInterface->ExportSet(ISXEQFPSXML,XMLFileName);

	pISInterface->UnloadSet(ISXEQFPSXML);
...
void ISXEQFPS::RegisterCommands()
{
	// add any commands
...
void ISXEQFPS::RegisterAliases()
{
	// add any aliases
}

void ISXEQFPS::RegisterDataTypes()
{
	// add any datatypes
	// pMyType = new MyType;
...
void ISXEQFPS::UnRegisterCommands()
{
	// remove commands
	//    pISInterface->RemoveCommand("MyCommand");
}
void ISXEQFPS::UnRegisterAliases()
{
	// remove aliases
}
void ISXEQFPS::UnRegisterDataTypes()
{
	// remove data types
	//if (pMyType)
	//{
...
void ISXEQFPS::UnRegisterTopLevelObjects()
{
	// remove Top-Level Objects
	//    pISInterface->RemoveTopLevelObject("MapSpawn");
}
void ISXEQFPS::UnRegisterServices()
{
	// shutdown our own services
	//    if (hISXEQFPSService)
	//        pISInterface->ShutdownService(this,hISXEQFPSService);
}

void ISXEQFPS::LoadSettings()
{
	bool bChanged = false;

	// open the settings file, and store the ID number in ISXEQFPSXML
	ISXEQFPSXML = pISInterface->OpenSettings(XMLFileName);
	...
	Any set can be referred to by its ID.  The base set is stored in ISXEQFPSXML.  Note that the settings
	...
	*/
	...
	if (unsigned int GeneralGUID = pISInterface->FindSet(ISXEQFPSXML, "General"))
	{
