...
#include "ISXEQMelee.h"

// The mandatory pre-setup function.  Our name is "ISXEQMelee", and the class is ISXEQMelee.
...
// our instanced class.
ISXPreSetup("ISXEQMelee", ISXEQMelee);
...
bool ISXEQMelee::Initialize(ISInterface *p_ISInterface)
{
	...
	WriteChatf("ISXEQMelee Loaded");
	return true;
}

// shutdown sequence
void ISXEQMelee::Shutdown()
{
	// save settings, if you changed them and want to save them now.  You should normally save
	// changes immediately.
	//pISInterface->ExportSet(ISXEQMeleeXML,XMLFileName);

	pISInterface->UnloadSet(ISXEQMeleeXML);
...
void ISXEQMelee::ConnectServices()
{
...
void ISXEQMelee::RegisterCommands()
{
	// add any commands
...
void ISXEQMelee::RegisterAliases()
{
	// add any aliases
}

void ISXEQMelee::RegisterDataTypes()
{
	// add any datatypes
	// pMyType = new MyType;
...
void ISXEQMelee::UnRegisterCommands()
{
	// remove commands
	//    pISInterface->RemoveCommand("MyCommand");
}
void ISXEQMelee::UnRegisterAliases()
{
	// remove aliases
}
void ISXEQMelee::UnRegisterDataTypes()
{
	// remove data types
	//if (pMyType)
	//{
...
void ISXEQMelee::UnRegisterTopLevelObjects()
{
	// remove Top-Level Objects
	//    pISInterface->RemoveTopLevelObject("MapSpawn");
}
void ISXEQMelee::UnRegisterServices()
{
	// shutdown our own services
	//    if (hISXEQMeleeService)
	//        pISInterface->ShutdownService(this,hISXEQMeleeService);
}

void ISXEQMelee::LoadSettings()
{
	bool bChanged = false;

	// open the settings file, and store the ID number in ISXEQMeleeXML
	ISXEQMeleeXML = pISInterface->OpenSettings(XMLFileName);
	...
	Any set can be referred to by its ID.  The base set is stored in ISXEQMeleeXML.  Note that the settings
	...
	*/
	...
	if (unsigned int GeneralGUID = pISInterface->FindSet(ISXEQMeleeXML, "General"))
	{
