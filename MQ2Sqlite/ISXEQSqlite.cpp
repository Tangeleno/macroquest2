...
#include "ISXEQSqlite.h"

// The mandatory pre-setup function.  Our name is "ISXEQSqlite", and the class is ISXEQSqlite.
...
// our instanced class.
ISXPreSetup("ISXEQSqlite", ISXEQSqlite);
...
bool ISXEQSqlite::Initialize(ISInterface *p_ISInterface)
{
	...
	WriteChatf("ISXEQSqlite Loaded");
	return true;
}

// shutdown sequence
void ISXEQSqlite::Shutdown()
{
	// save settings, if you changed them and want to save them now.  You should normally save
	// changes immediately.
	//pISInterface->ExportSet(ISXEQSqliteXML,XMLFileName);

	pISInterface->UnloadSet(ISXEQSqliteXML);
...
void ISXEQSqlite::ConnectServices()
{
...
void ISXEQSqlite::RegisterCommands()
{
	// add any commands
...
void ISXEQSqlite::RegisterAliases()
{
	// add any aliases
}

void ISXEQSqlite::RegisterDataTypes()
{
	// add any datatypes
	// pMyType = new MyType;
...
void ISXEQSqlite::UnRegisterCommands()
{
	// remove commands
	//    pISInterface->RemoveCommand("MyCommand");
}
void ISXEQSqlite::UnRegisterAliases()
{
	// remove aliases
}
void ISXEQSqlite::UnRegisterDataTypes()
{
	// remove data types
	//if (pMyType)
	//{
...
void ISXEQSqlite::UnRegisterTopLevelObjects()
{
	// remove Top-Level Objects
	//    pISInterface->RemoveTopLevelObject("MapSpawn");
}
void ISXEQSqlite::UnRegisterServices()
{
	// shutdown our own services
	//    if (hISXEQSqliteService)
	//        pISInterface->ShutdownService(this,hISXEQSqliteService);
}

void ISXEQSqlite::LoadSettings()
{
	bool bChanged = false;

	// open the settings file, and store the ID number in ISXEQSqliteXML
	ISXEQSqliteXML = pISInterface->OpenSettings(XMLFileName);
	...
	Any set can be referred to by its ID.  The base set is stored in ISXEQSqliteXML.  Note that the settings
	...
	*/
	...
	if (unsigned int GeneralGUID = pISInterface->FindSet(ISXEQSqliteXML, "General"))
	{
