...
// The mandatory pre-setup function.  Our name is "ISXEQEQBC", and the class is ISXEQEQBC.
...
// our instanced class.
ISXPreSetup("ISXEQEQBC", ISXEQEQBC);
...
bool ISXEQEQBC::Initialize(ISInterface *p_ISInterface)
{
	...
	WriteChatf("ISXEQEQBC Loaded");
	return true;
}

// shutdown sequence
void ISXEQEQBC::Shutdown()
{
	// save settings, if you changed them and want to save them now.  You should normally save
	// changes immediately.
	//pISInterface->ExportSet(ISXEQEQBCXML,XMLFileName);

	pISInterface->UnloadSet(ISXEQEQBCXML);
...
void ISXEQEQBC::ConnectServices()
{
...
void ISXEQEQBC::RegisterCommands()
{
	// add any commands
...
void ISXEQEQBC::RegisterAliases()
{
	// add any aliases
}

void ISXEQEQBC::RegisterDataTypes()
{
	// add any datatypes
	// pMyType = new MyType;
...
void ISXEQEQBC::UnRegisterCommands()
{
	// remove commands
	//    pISInterface->RemoveCommand("MyCommand");
}
void ISXEQEQBC::UnRegisterAliases()
{
	// remove aliases
}
void ISXEQEQBC::UnRegisterDataTypes()
{
	// remove data types
	//if (pMyType)
	//{
...
void ISXEQEQBC::UnRegisterTopLevelObjects()
{
	// remove Top-Level Objects
	//    pISInterface->RemoveTopLevelObject("MapSpawn");
}
void ISXEQEQBC::UnRegisterServices()
{
	// shutdown our own services
	//    if (hISXEQEQBCService)
	//        pISInterface->ShutdownService(this,hISXEQEQBCService);
}

void ISXEQEQBC::LoadSettings()
{
	bool bChanged = false;

	// open the settings file, and store the ID number in ISXEQEQBCXML
	ISXEQEQBCXML = pISInterface->OpenSettings(XMLFileName);
	...
	Any set can be referred to by its ID.  The base set is stored in ISXEQEQBCXML.  Note that the settings
	...
	*/
	...
	if (unsigned int GeneralGUID = pISInterface->FindSet(ISXEQEQBCXML, "General"))
	{
