...
#include "ISXEQFeedme.h"

// The mandatory pre-setup function.  Our name is "ISXEQFeedme", and the class is ISXEQFeedme.
...
// our instanced class.
ISXPreSetup("ISXEQFeedme", ISXEQFeedme);
...
bool ISXEQFeedme::Initialize(ISInterface *p_ISInterface)
{
	...
	WriteChatf("ISXEQFeedme Loaded");
	return true;
}

// shutdown sequence
void ISXEQFeedme::Shutdown()
{
	// save settings, if you changed them and want to save them now.  You should normally save
	// changes immediately.
	//pISInterface->ExportSet(ISXEQFeedmeXML,XMLFileName);

	pISInterface->UnloadSet(ISXEQFeedmeXML);
...
void ISXEQFeedme::ConnectServices()
{
...
void ISXEQFeedme::RegisterCommands()
{
	// add any commands
...
void ISXEQFeedme::RegisterAliases()
{
	// add any aliases
}

void ISXEQFeedme::RegisterDataTypes()
{
	// add any datatypes
	// pMyType = new MyType;
...
void ISXEQFeedme::UnRegisterCommands()
{
	// remove commands
	//    pISInterface->RemoveCommand("MyCommand");
}
void ISXEQFeedme::UnRegisterAliases()
{
	// remove aliases
}
void ISXEQFeedme::UnRegisterDataTypes()
{
	// remove data types
	//if (pMyType)
	//{
...
void ISXEQFeedme::UnRegisterTopLevelObjects()
{
	// remove Top-Level Objects
	//    pISInterface->RemoveTopLevelObject("MapSpawn");
}
void ISXEQFeedme::UnRegisterServices()
{
	// shutdown our own services
	//    if (hISXEQFeedmeService)
	//        pISInterface->ShutdownService(this,hISXEQFeedmeService);
}

void ISXEQFeedme::LoadSettings()
{
	bool bChanged = false;

	// open the settings file, and store the ID number in ISXEQFeedmeXML
	ISXEQFeedmeXML = pISInterface->OpenSettings(XMLFileName);
	...
	Any set can be referred to by its ID.  The base set is stored in ISXEQFeedmeXML.  Note that the settings
	...
	*/
	...
	if (unsigned int GeneralGUID = pISInterface->FindSet(ISXEQFeedmeXML, "General"))
	{
