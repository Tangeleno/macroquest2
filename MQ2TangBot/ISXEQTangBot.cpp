...
#include "ISXEQTangBot.h"

// The mandatory pre-setup function.  Our name is "ISXEQTangBot", and the class is ISXEQTangBot.
...
// our instanced class.
ISXPreSetup("ISXEQTangBot", ISXEQTangBot);
...
unsigned int ISXEQTangBotXML = 0;
...
bool ISXEQTangBot::Initialize(ISInterface *p_ISInterface)
{
	...
	WriteChatf("ISXEQTangBot Loaded");
	return true;
}

// shutdown sequence
void ISXEQTangBot::Shutdown()
{
	// save settings, if you changed them and want to save them now.  You should normally save
	// changes immediately.
	//pISInterface->ExportSet(ISXEQTangBotXML,XMLFileName);

	pISInterface->UnloadSet(ISXEQTangBotXML);
...
void ISXEQTangBot::ConnectServices()
{
...
void ISXEQTangBot::RegisterCommands()
{
	// add any commands
...
void ISXEQTangBot::RegisterAliases()
{
	// add any aliases
}

void ISXEQTangBot::RegisterDataTypes()
{
	// add any datatypes
	// pMyType = new MyType;
...
void ISXEQTangBot::UnRegisterCommands()
{
	// remove commands
	//    pISInterface->RemoveCommand("MyCommand");
}
void ISXEQTangBot::UnRegisterAliases()
{
	// remove aliases
}
void ISXEQTangBot::UnRegisterDataTypes()
{
	// remove data types
	//if (pMyType)
	//{
...
void ISXEQTangBot::UnRegisterTopLevelObjects()
{
	// remove Top-Level Objects
	//    pISInterface->RemoveTopLevelObject("MapSpawn");
}
void ISXEQTangBot::UnRegisterServices()
{
	// shutdown our own services
	//    if (hISXEQTangBotService)
	//        pISInterface->ShutdownService(this,hISXEQTangBotService);
}

void ISXEQTangBot::LoadSettings()
{
	bool bChanged = false;

	// open the settings file, and store the ID number in ISXEQTangBotXML
	ISXEQTangBotXML = pISInterface->OpenSettings(XMLFileName);
	...
	Any set can be referred to by its ID.  The base set is stored in ISXEQTangBotXML.  Note that the settings
	...
	*/
	...
	if (unsigned int GeneralGUID = pISInterface->FindSet(ISXEQTangBotXML, "General"))
	{
