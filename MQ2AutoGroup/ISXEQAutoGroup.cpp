...
#include "ISXEQAutoGroup.h"

// The mandatory pre-setup function.  Our name is "ISXEQAutoGroup", and the class is ISXEQAutoGroup.
...
// our instanced class.
ISXPreSetup("ISXEQAutoGroup", ISXEQAutoGroup);
...
unsigned int ISXEQAutoGroupXML = 0;
...
bool ISXEQAutoGroup::Initialize(ISInterface *p_ISInterface)
{
	...
	WriteChatf("ISXEQAutoGroup Loaded");
	return true;
}

// shutdown sequence
void ISXEQAutoGroup::Shutdown()
{
	// save settings, if you changed them and want to save them now.  You should normally save
	// changes immediately.
	//pISInterface->ExportSet(ISXEQAutoGroupXML,XMLFileName);

	pISInterface->UnloadSet(ISXEQAutoGroupXML);
...
void ISXEQAutoGroup::ConnectServices()
{
...
void ISXEQAutoGroup::RegisterCommands()
{
	// add any commands
...
void ISXEQAutoGroup::RegisterAliases()
{
	// add any aliases
}

void ISXEQAutoGroup::RegisterDataTypes()
{
	// add any datatypes
	// pMyType = new MyType;
...
void ISXEQAutoGroup::UnRegisterCommands()
{
	// remove commands
	//    pISInterface->RemoveCommand("MyCommand");
}
void ISXEQAutoGroup::UnRegisterAliases()
{
	// remove aliases
}
void ISXEQAutoGroup::UnRegisterDataTypes()
{
	// remove data types
	//if (pMyType)
	//{
...
void ISXEQAutoGroup::UnRegisterTopLevelObjects()
{
	// remove Top-Level Objects
	//    pISInterface->RemoveTopLevelObject("MapSpawn");
}
void ISXEQAutoGroup::UnRegisterServices()
{
	// shutdown our own services
	//    if (hISXEQAutoGroupService)
	//        pISInterface->ShutdownService(this,hISXEQAutoGroupService);
}

void ISXEQAutoGroup::LoadSettings()
{
	bool bChanged = false;

	// open the settings file, and store the ID number in ISXEQAutoGroupXML
	ISXEQAutoGroupXML = pISInterface->OpenSettings(XMLFileName);
	...
	Any set can be referred to by its ID.  The base set is stored in ISXEQAutoGroupXML.  Note that the settings
	...
	*/
	...
	if (unsigned int GeneralGUID = pISInterface->FindSet(ISXEQAutoGroupXML, "General"))
	{
