...
#include "ISXEQNetheal.h"

// The mandatory pre-setup function.  Our name is "ISXEQNetheal", and the class is ISXEQNetheal.
...
// our instanced class.
ISXPreSetup("ISXEQNetheal", ISXEQNetheal);
...
unsigned int ISXEQNethealXML = 0;
...
bool ISXEQNetheal::Initialize(ISInterface *p_ISInterface)
{
	...
	WriteChatf("ISXEQNetheal Loaded");
	return true;
}

// shutdown sequence
void ISXEQNetheal::Shutdown()
{
	// save settings, if you changed them and want to save them now.  You should normally save
	// changes immediately.
	//pISInterface->ExportSet(ISXEQNethealXML,XMLFileName);

	pISInterface->UnloadSet(ISXEQNethealXML);
...
void ISXEQNetheal::ConnectServices()
{
...
void ISXEQNetheal::RegisterCommands()
{
	// add any commands
...
void ISXEQNetheal::RegisterAliases()
{
	// add any aliases
}

void ISXEQNetheal::RegisterDataTypes()
{
	// add any datatypes
	// pMyType = new MyType;
...
void ISXEQNetheal::UnRegisterCommands()
{
	// remove commands
	//    pISInterface->RemoveCommand("MyCommand");
}
void ISXEQNetheal::UnRegisterAliases()
{
	// remove aliases
}
void ISXEQNetheal::UnRegisterDataTypes()
{
	// remove data types
	//if (pMyType)
	//{
...
void ISXEQNetheal::UnRegisterTopLevelObjects()
{
	// remove Top-Level Objects
	//    pISInterface->RemoveTopLevelObject("MapSpawn");
}
void ISXEQNetheal::UnRegisterServices()
{
	// shutdown our own services
	//    if (hISXEQNethealService)
	//        pISInterface->ShutdownService(this,hISXEQNethealService);
}

void ISXEQNetheal::LoadSettings()
{
	bool bChanged = false;

	// open the settings file, and store the ID number in ISXEQNethealXML
	ISXEQNethealXML = pISInterface->OpenSettings(XMLFileName);
	...
	Any set can be referred to by its ID.  The base set is stored in ISXEQNethealXML.  Note that the settings
	...
	*/
	...
	if (unsigned int GeneralGUID = pISInterface->FindSet(ISXEQNethealXML, "General"))
	{
