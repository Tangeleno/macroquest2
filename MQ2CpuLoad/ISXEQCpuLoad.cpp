...
#include "ISXEQCpuLoad.h"

// The mandatory pre-setup function.  Our name is "ISXEQCpuLoad", and the class is ISXEQCpuLoad.
...
// our instanced class.
ISXPreSetup("ISXEQCpuLoad", ISXEQCpuLoad);
...
unsigned int ISXEQCpuLoadXML = 0;
...
bool ISXEQCpuLoad::Initialize(ISInterface *p_ISInterface)
{
	...
	WriteChatf("ISXEQCpuLoad Loaded");
	return true;
}

// shutdown sequence
void ISXEQCpuLoad::Shutdown()
{
	// save settings, if you changed them and want to save them now.  You should normally save
	// changes immediately.
	//pISInterface->ExportSet(ISXEQCpuLoadXML,XMLFileName);

	pISInterface->UnloadSet(ISXEQCpuLoadXML);
...
void ISXEQCpuLoad::ConnectServices()
{
...
void ISXEQCpuLoad::RegisterCommands()
{
	// add any commands
...
void ISXEQCpuLoad::RegisterAliases()
{
	// add any aliases
}

void ISXEQCpuLoad::RegisterDataTypes()
{
	// add any datatypes
	// pMyType = new MyType;
...
void ISXEQCpuLoad::UnRegisterCommands()
{
	// remove commands
	//    pISInterface->RemoveCommand("MyCommand");
}
void ISXEQCpuLoad::UnRegisterAliases()
{
	// remove aliases
}
void ISXEQCpuLoad::UnRegisterDataTypes()
{
	// remove data types
	//if (pMyType)
	//{
...
void ISXEQCpuLoad::UnRegisterTopLevelObjects()
{
	// remove Top-Level Objects
	//    pISInterface->RemoveTopLevelObject("MapSpawn");
}
void ISXEQCpuLoad::UnRegisterServices()
{
	// shutdown our own services
	//    if (hISXEQCpuLoadService)
	//        pISInterface->ShutdownService(this,hISXEQCpuLoadService);
}

void ISXEQCpuLoad::LoadSettings()
{
	bool bChanged = false;

	// open the settings file, and store the ID number in ISXEQCpuLoadXML
	ISXEQCpuLoadXML = pISInterface->OpenSettings(XMLFileName);
	...
	Any set can be referred to by its ID.  The base set is stored in ISXEQCpuLoadXML.  Note that the settings
	...
	*/
	...
	if (unsigned int GeneralGUID = pISInterface->FindSet(ISXEQCpuLoadXML, "General"))
	{
